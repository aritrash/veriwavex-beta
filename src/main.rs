/*
 * VeriWaveX - Professional Verilog Simulation Suite
 * Version: 2.0.0 (Linux/Windows Cross-Platform)
 * * Copyright (c) 2026 Aritrash Sarkar. All Rights Reserved.
 * Unauthorized copying of this file, via any medium is strictly prohibited.
 * Proprietary and confidential.
 * * Written by Aritrash Sarkar <aritrashsarkar@gmail.com>, April 2026
 */

#![windows_subsystem = "windows"]

// --- 1. MODULE DECLARATIONS ---
mod models;
mod logic;
mod app;
mod ui;

use eframe::egui;

fn main() -> eframe::Result<()> {
    // --- 2. ICON LOADING ---
    // We load the icon at the very start to pass it to the native options
    let icon_bytes = include_bytes!("../assets/logo.png");
    let icon = image::load_from_memory(icon_bytes).ok().map(|img| {
        let rgba = img.to_rgba8();
        let (width, height) = rgba.dimensions();
        egui::IconData {
            rgba: rgba.into_raw(),
            width,
            height,
        }
    });

    // --- 3. NATIVE OPTIONS ---
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_maximized(true)
            .with_inner_size([1200.0, 800.0])
            .with_icon(icon.unwrap_or_default()),
        ..Default::default()
    };

    // --- 4. EXECUTION ---
    eframe::run_native(
        "VeriWaveX v2.0.0",
        options,
        Box::new(|cc| {
            // Required for loading our splash.png and logo.png
            egui_extras::install_image_loaders(&cc.egui_ctx);
            
            // Start the application using our modular app.rs
            Ok(Box::new(app::VerilogApp::new(cc)))
        }),
    )
}